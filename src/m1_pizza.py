...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  20 red dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 50)
    draw_points_on_circle(window, circle, 20, 'red')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        little_circles += [rg.Circle(points[k], 10)]
        little_circles[k].fill_color = color
        little_circles[k].attach_to(window)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 1:  800 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 3
    pizza(window, circle, 800, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        lines += [rg.Line(circle.center, points[k])]
        lines[k].color = color
        lines[k].thickness = thickness
        lines[k].attach_to(window)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(300, 300), 200)
    circle.fill_color = 'purple'
    fancy_polygon(window, circle, 51, 25, 'lime green', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    lines = []
    point_index = 0
    next_point_index = hops_to_next_point


    for k in range(number_of_lines):
        if point_index >= len(points):
            point_index = point_index - len(points)
        if next_point_index >= len(points):
            next_point_index = next_point_index - len(points)

        lines += [rg.Line(points[point_index], points[next_point_index])]

        point_index += hops_to_next_point
        next_point_index += hops_to_next_point
